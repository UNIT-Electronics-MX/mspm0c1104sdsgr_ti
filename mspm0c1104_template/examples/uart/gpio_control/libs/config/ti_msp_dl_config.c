...
void SYSCFG_DL_GPIO_init(void)
{
    ...
    /* Configure Button (PA1) as input with internal pull-up */
    DL_GPIO_initDigitalInputFeatures(GPIO_BUTTON_IOMUX,
        DL_GPIO_INVERSION_DISABLE, DL_GPIO_RESISTOR_PULL_UP,
        DL_GPIO_HYSTERESIS_DISABLE, DL_GPIO_WAKEUP_DISABLE);
    
    /* Configure LED (PA0) as output - Open Drain requires external pull-up */
    DL_GPIO_initDigitalOutput(GPIO_LED_IOMUX);
    DL_GPIO_clearPins(GPIO_LED_PORT, GPIO_LED_PIN);  // Start with LED OFF
