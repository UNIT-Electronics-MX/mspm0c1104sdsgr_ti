...
- **`main.c`**: Main application file with LED blink example on PA24
...
2. **Update the Makefile** - Add new files to `SOURCE_FILES` variable:
   ```makefile
   SOURCE_FILES = src/main.c src/your_new_file.c libs/config/ti_msp_dl_config.c startup_mspm0c110x_gcc.c
   ```
...
The `src/` directory is automatically included in the compiler include path via the Makefile `-Isrc` flag.
